...
### 全量门槛 `npm run check` 的既有失败（2026-09-18 记录）

跑全量门槛时有若干 live 场景不通过。**与方案这一轮的改动无关** —— 这一轮改的是
git 集成 / 设置 / 来源 / 插件，而失败的是任务面板、上下文压缩、子代理、文件树与
布局测量：

- **两次连跑的失败清单不一致**（7 个 vs 12 个），说明里面有并发/环境相关的成分
- 抽查 `todos`：失败在「进度条推进动画 / 当前那条 active / 行内显示『正在进行』」，
  与 git 集成没有任何交集（HANDOFF 开头也写明「任务面板保持现状」）
- 这一轮**修掉的过时断言**：`narrow` / `resize` / `topbar` / `panels` / `gitreview`
  里关于「48px 紧凑轨」与「没有 gh 就说无法获取」的那几条 —— 紧凑轨
  （`.rail-compact`）早已从源码移除，断言还在找它
- **已知布局遗留**：窄窗口（≤1002px）下左栏收起后**仍残留 48px**（宽窗口是 0）。
  那是紧凑轨移除时没清掉的那一列宽度。`narrow` 的断言已改成意图级并**打印实测
  值**，不让它假装是 0；要彻底修需要动 `rail.css` 的列定义
