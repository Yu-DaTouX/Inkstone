...
## 2026-09-24 · Inkstone 仓库命名与根目录整理

| 六栏 | 当前证据 |
|---|---|
| 实现 | 用户授权后将 GitHub 仓库命名为 Inkstone，并更新 About；README、使用指南、package 仓库链接及本地 origin 同步新地址。三份根目录审计脚本移至 scripts/audit，npm 命令入口同步；移除 goal-plan.json、tmp-image-capability-query.json、nul 的版本跟踪并加入忽略规则，本地临时内容保留。 |
| 自动检查 | 本轮仅核对 Git 索引变更与远端元数据，不运行产品测试。 |
| 真实运行 | 不涉及产品运行；审计脚本迁移后未执行。 |
| 视觉验收 | 不更改页面布局和品牌素材。 |
| 应用与包 | 提交范围限于仓库整理；其他未提交的源码和文档改动保留。 |
| 剩余限制 | 历史提交仍保留旧文件；本轮不改写 Git 历史，不构成全仓敏感信息审计。 |

## 2026-09-24 · Inkstone 品牌命名与 GitHub 产品主页

| 六栏 | 当前证据 |
|---|---|
| 实现 | 按用户确定的英文名 Inkstone 重写中文 README，新增英文 README、中英文用户指南、`docs/design/BRAND.md`、深浅 SVG 字标与 UI 演示截图，以及 `scripts/build-brand-assets.ps1` 换色生成脚本。DESIGN 与文档索引仅增品牌/指南入口。按用户提供的品牌原图经内置 imagegen 编辑并接入 3:1 纸墨横幅；截图演示配置改为 GPT-6 Astra Max（max 档），中英文主页与指南统一为应用内登录，无需终端。主页采用“让想法成形。 / Give ideas form.”，开发说明折叠在末尾，内部交接和验收信息不进入产品介绍正文。 |
| 自动检查 | 本轮最新 41 个本地链接 / 锚点均有效；SVG XML 与纯路径结构有效；定向 `git diff --check` 通过。全仓引用审计 `brokenDocLinks=[]`；其它历史命令/原文引用提示仍保留，不宣称全仓审计无问题。 |
| 真实运行 | 页面配图由既有开发构建的真实 renderer 在独立 userData 中渲染合成书店案例；深浅 main 两组关键元素完整、横向溢出 0，未调用外部模型、未读取真实会话。截图是产品展示，不能证明案例中的任务真正执行。 |
| 视觉验收 | 已查看深浅主页、英文主页、390px 窄屏和页尾截图；图片无破图，窄屏横向溢出 0；本地 HTML 的深浅切换可切换并恢复原素材。预览按 GitHub README 排版模拟，不冒充 GitHub 线上渲染验收。 |
| 应用与包 | 只改主页、说明文档和独立品牌素材。未构建产品、未更改应用名称/CLI/数据目录、未重启用户窗口、未打包；用户已授权将本次品牌主页改版提交并推送 GitHub。 |
| 剩余限制 | 本次提交范围限于品牌主页及对应文档；About 文案保存在 BRAND 中，远端设置未修改。中性深浅色为可换色默认值，用户未指定最终品牌色。远端发行版可能落后开发分支，README 已说明。 |
...
| 六栏 | 证据 |
|---|---|
| 实现 | ① 行内代码不再用彩色：`--code-inline-fg` 深色 `#7ee7f7` → **`#cdd6dc`**、浅色 `#0a5fa8` → **`#45525a`**，底色改中性（`rgba(255,255,255,.07)` / `rgba(0,0,0,.055)`）—— 靠等宽 + 淡底区分，不靠颜色。② 主题切换改两个方向、**760ms**（原 380ms 单向）：切深色 = 新层从中心向外晕开（`theme-spread`），切浅色 = **旧层**从外缘向中心收拢（`theme-collapse`，它要压在新层上面，否则动画被盖住），方向由 `<html data-theme-dir>`（`out` / `in`）驱动、在 `startViewTransition` **之前**写；两个方向都只让一层动。令牌意图先写进 `DESIGN.md`（§2.6 行内代码、§5 动效）再同步 `tokens.css` / `shell.css`。 |
| 自动检查 | `npm run typecheck`（含 CSS 约定与 layer 自检）/ `npm run build` 通过；`npm run test:unit` **4682/4682**。 |
| 真实运行 | `npm run test:live -- light`（cost 0）**全绿**，新增断言：四次真实切换（走设置面板同一条 `yan:theme` 事件入口）里深色拿到 `theme-spread`、浅色拿到 `theme-collapse`，方向标记分别是 `out` / `in`；CSSOM 里两条动画挂在正确的层（new / old）且都是 760ms。 |
| 视觉验收 | `npm run visual:matrix -- 2 3` 通过并逐张看图 —— 深浅两套下行内代码都是灰底小标签、不抢正文：`matrix-main-940x620-100-{dark,light}-2026-09-23-1808.png`。fixture 补了行内代码样本（用户消息与助手正文各一处），否则截图里根本没有这种字。 |
| 应用与包 | 未重启用户现有窗口，未重跑解包 / 便携 / 安装包。 |
| 剩余限制 | 动画观感（扩散 / 收拢是否顺眼、760ms 是否偏慢）仍需用户在本机看一眼；`handoffnote` / `handofftally` 等旧截图还是旧配色，未全部重跑。 |

顺带修掉的**探针**缺陷：`light.js` 的「浅色下不该有深色块」用 `match(/\d+/g)` 解析颜色，碰到 `color-mix()` 的 computed 值 `color(srgb …)` 会把字面的 `srgb` 里的 0 当成红色通道 —— 浅色面板被读成纯黑（实测误报过 `.goal-panel`）。现在统一走 `parseColor()`（`color(srgb …)` 与 `rgba()` 两种写法都支持），该断言由红转绿。
