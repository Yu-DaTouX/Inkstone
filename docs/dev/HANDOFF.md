...
| 阶段 | 状态 | 剩余工作 | 依赖 |
|---|---|---|---|
| **G3 获取远程 / 推送 / PR** | **除 PR 主路径外已做**（2026-09-18） | 已做：推送 / 拉取（G2）、**远程比较**（用 upstream 做 range，不联网也能用）、**托管网页比较**（只认 github / gitlab / bitbucket，且三者路径不同 —— Bitbucket 的顺序与我们相反；认不出的自建服务**不给链接**）。**未做**：PR 状态主路径 —— 本机没有 gh（实测），按方案的降级路径显示「无法获取 Pull Request 状态」；真要接 PR 需要**具备权限的测试仓库**单独验收，本地 bare remote 不能替代 | 一个有权限的远程仓库 |
| **W1 创建并打开用户工作树** | **数据层 + 菜单界面已做**（2026-09-18） | 已做：建在仓库旁边的长期目录、三类删除拦截（未提交 / 未推送 / 有任务在跑）+ 主工作树与 locked 不可删、路径必须是 git 登记过的工作树。**未做**：创建成功后**自动登记为项目**（当前要在项目里手动打开那个路径）、工作树的分组与关联任务 | — |
...
| **S1 来源与附件菜单** | **核查已完成，实现未开始**（2026-09-18） | **核查结论（这是方案 §8 要求的第一步）：现有附件链【不持久化】** —— `Attachment` 是渲染端 store 的临时状态：图片是内存里的 base64（关掉应用就没了），文件引用**只存 path**（没有内容也没有授权记录）。所以方案那句「不能只保存会被清理的临时路径」在现状下**不成立**。要做 S1，先得有**持久化资源引用**：主进程把附件写到数据目录（`<数据目录>/sources/<sessionId>/<内容哈希>.<ext>`）、返回引用与内容指纹，来源模型再带上 sourceId / sessionId / kind / 标题 / 原始定位 / 获取状态 / 可选的消息关联。界面按方案分「已关联 / 已读取 / 本轮已参与上下文」三态（证据不足时不显示后两态），移除引用**不删用户原文件**、不改写已发送的历史 | 现有附件链改造（先做持久化） |
| **P1 / P2 pi 插件目录与包管理** | **P1 前置核查已完成 + 目录入口已实现（验证未完成）；P2 未开始**（2026-09-18） | **核查结论（方案 §9 的前置要求）**：目录实测 **5426 个包**，字段有包名 / 描述 / 作者 / 月下载量 / 更新时间 / 类型（extension / skill / theme / prompt）/ npm 与 repo 链接 / **安装命令 `pi install npm:<包名>`**；筛选与排序**全部在服务端**，页面是服务端渲染的 HTML —— **没有结构化数据接口**。所以方案里「确认目录有稳定数据接口后再做原生搜索」的**条件不成立**：按方案的正确做法是**不做**原生搜索（做了也会立刻过期）。已实现：设置「关于」里的**目录入口**（内置浏览器打开，并写明「目录没有结构化数据接口」）。**验证未完成**：这条入口的 live 断言（在 `panels` 探针里定位设置面板 tab）没有通过，为避免把红断言留在门槛里已从探针移除 —— 它现在只过了 typecheck，**按本项目的六栏口径不算完成**。P2（安装 / 启用 / 更新 / 卸载）没动：那会写用户的 pi 目录（`PI_AGENT_DIR`），且要按方案分别记录「安装成功 / 加载成功 / 当前会话可用」并安排任务运行中的安全生效时机 | P1 的 live 验证；P2 需要写用户 pi 目录的完整设计 |
