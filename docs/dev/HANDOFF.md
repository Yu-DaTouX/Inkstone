...
| 阶段 | 状态 | 剩余工作 | 依赖 |
|---|---|---|---|
| **G3 获取远程 / 推送 / PR** | **推送与拉取已做**（G2 内），**PR 与远程比较未做** | 本机**没有 `gh`**（实测），PR 状态按方案的降级路径显示「无法获取」；真要接 PR 需要**具备权限的测试仓库**单独验收，本地 bare remote 不能替代 | 需要一个有权限的远程仓库 |
| **W1 创建并打开用户工作树** | 未开始 | 工作树的独立生命周期（**不复用**子代理的强制清理函数）、目标目录登记、源工作区与 index 保持不变 | G1 / G2 |
| **W2 携带未提交改动 + 带会话继续** | 未开始 | 前置是源任务落盘 + 权限 / 相对路径 / 附件授权 / 上下文派生的重绑定；做不到就只开「在新工作树开始新会话」，不显示「无缝继续」 | W1 |
