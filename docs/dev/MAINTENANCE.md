...
✓ 回合真的跑起来了（否则下面"没触发压缩"证明不了任何事）
  消息数=10｜isAgentRunning=false｜pi 用量=1942/1048576（percent=0.185…）｜policyState={…}
```

**规则**：凡是「等某个副作用出现」的探针，都要先证明**产生副作用的前提条件**成立，
并把前提条件的数值打出来。前提不成立时直接 `return`，别让后续断言产生误导性结论。

## pi 的 `contextUsage.tokens` 只算消息文本（pi 0.85.1 实测）

| 发出的消息 | pi 报的 `contextUsage.tokens` |
|---|---|
| 15 字符（`只回复「好」，不要用任何工具。`） | **4**（= 15 ÷ 4） |
| ~7.8k 字符（填充文本 ×250） | **1940**（≈ 7750 ÷ 4） |

它**不含** system prompt 与工具定义。两个后果：

1. 想用「把工作集压到 1500」来验证接管，**消息本身**必须 ≥ 6000 字符，否则用量到不了线；
2. 界面上小会话的「用量占比」会显著偏小（分母是完整窗口、分子只有消息文本）。

另外 pi 会**拒绝压缩"太小"的会话**：`Compaction failed: Nothing to compact (session too small)` ——
所以验证接管的场景必须先跑一个**预热回合**建立历史。
