...
当前不是“从头盘点项目”，而是按 **P0 → P1** 继续收口：04-S6b-2 的一个真实 Skill 文件候选整链已在本轮用户明确授权下通过；01-S5 的 `question` 唯一入口也已迁到宿主 `yan question ask`，当前 P0 主要剩其它资源类型的包级候选证据、`context_recall` / 旧入口边界，以及它们完成后的 09 最终发布门槛；P1 是 06 的残余证据边界、11 Harness / 工作窗口 / 1M 队列、10 最终视觉矩阵和 08 Android 客户端。
...
| P0-2 | 01 默认 pi 架构迁移 | `question` 的最终唯一入口已在本轮收口为宿主 `yan question ask`；S5 仍需按当前源码处理 `context_recall`、旧兼容入口与空壳扩展 | 默认 pi + 允许的砚薄适配层可启动、扩展边界唯一、旧入口不再用户可见 |
...
重点验证：默认启动、扩展加载边界、`yan` 正式入口、旧入口不存在、项目知识 / 任务 / 浏览器 / context 归属没有回归；`question` 已有宿主 CLI 的六栏证据，剩余唯一模型工具 carrier 是 `context_recall`，最终把它和“明确不迁移”的项写入六栏。
