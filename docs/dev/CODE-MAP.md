...
| `scripts/bench/context-bench.mjs` | 250 | N21-9 **跑批器**（`npm run bench:context`）：四策略 × 任务集各跑一次 `pi --print`，收交付物 → 调 `context-bench.ts` 的判分与对比 → 写报告 JSON；`--mock`（假 provider，cost 0，只验装置）/ `--live`（真实模型，要额度） | 不进自动门槛（live 要额度）；mock 模式可随时手动跑 |
...
| `resources/pi-extensions/work-mode.js` | 150 | **澄清档的工具门禁（实施-05 S3a）**：每轮 `before_agent_start` 读模式快照 —— `clarify` 时 `setActiveTools` 只留 `read/grep/find/ls/question/context_recall/bash`（其余从表里拿掉，模型看不见），`tool_call` 兑底拒绝；bash **只放行** `yan goal status\|ready\|report` 形状且禁 shell 元字符。基集只在第一次记录（宿主 `--tools` 决定），切回标准档恢复。为何在薄层：`setActiveTools` 只有扩展 API，RPC **没有**工具面（[证据-04-S1](../plan/证据-04-S1-技术预检.md) §3.2） | 宿主写快照；`hook-probe workmode\|workmode-standard\|workmode-allow`；`YAN_WORK_MODE_EXT_LOG` 诊断；[证据-05-S3a](../plan/证据-05-S3a-门禁与就绪转移.md) |
| `resources/pi-extensions/goal-resume.js` | 192 | **内部续行（实施-05 S3b 就绪开工 / S3c 自主档接着干 / S5c 模型出错后自动继续）**：`message_end`（助手消息、无工具调用）→ **1.8s 二次确认**（期间用户又发话 / 又调工具就放弃）→ 写 `goal-resume/<runnerId>.consumed.json`（**消费证据先落盘**）→ `pi.appendEntry('yan-goal-resume')` 留痕 → `pi.sendMessage({customType,content,display},{triggerTurn:true})`：按记录里的 `kind` 选 `yan-goal-ready` / `yan-goal-continue` / `yan-auto-continue`（S5c，无 `kind` 的旧快照一律当 `ready`），一条 **`custom` 角色**消息 + 触发一次真实回合。为何在薄层：`custom` 消息与 `triggerTurn` 只有扩展 API 有（RPC 没有），而 `custom` 角色正是「不是用户说的话」的机器可判形式 | 宿主写快照（`writeGoalResumeSnapshot`）；`yan:abort` / `yan:setWorkMode` 撤锪；live `goal` 第 7 节 / `goalloop` / `autocontinue`；`YAN_GOAL_RESUME_EXT_LOG` 诊断 |
...
| `shared/auto-continue.ts` | 217 | **自动继续的判定纯逻辑**（实施-05 S5c）：`classifyModelError`（额度 / 限流、认证 / 权限、上下文超限、用户取消**四类拦下**；认不出的当可重试 —— 有上限与退避兜着时这个方向的错判成本更低）/ `AUTO_CONTINUE_LIMIT = 3` / `AUTO_CONTINUE_DELAYS_MS = [3s, 10s, 30s]` / `planAutoContinue`（用户停止 → 不值得重试 → 到上限 → 继续）/ `retryResumeSummary`（续行正文，**必须**提醒「先检查再动手」防重复副作用）/ `isDuplicateError` / `sanitizeAutoContinueOptions`（`YAN_AUTO_CONTINUE` 测试通道）。**不碰文件与 IPC** | `main/auto-continue-service.ts`、`main/index.ts`；单测 `test-auto-continue.mjs` |
...
| `main/goal-service.ts` | 527 | **会话级目标存储**：`YAN_DIR/goals.json`，条目按会话（**键 = 会话文件路径**，与模式同一条理由）；`commitReady` **先查幂等再校验**（重放返回已提交结果，不推进 revision）、返回前已 `persist`（先落盘再续行的前提）；脏 JSON 不抛、落盘失败**必须抛**且内存态回退；`stop()` 供用户停止；S3c 新增 `armContinue`（只在「已在推进 + 报告过 + 未到上限」三条同时成立时 arm，到上限回 `reason:'limit'` 供宿主告知模型）与 `resetAutoContinues`（用户发言归零），`report` 进终态时**同一次落盘**清续行 | `main/index.ts` 的 `goalCapabilityHost` / `yan:getGoal`；单测 `test-goal.mjs`；live `goal` |
| `renderer/.../chat/Composer.tsx#WorkModePicker` | — | 菜单：三档 + 一句说明；键盘 ↑↓ / Enter / Space / Esc / Tab 离开；`Tab` 快切（`workModeTab !== false`，且 IME / 补全候选 / 长文模式均让路）；无补全时 `Esc` 把焦点送到模式按钮；自主档才上运行光带（`.composer-wrap[data-autonomous=1]`） | `store.setWorkMode` → IPC；设置页 `set-default-work-mode` / `set-work-mode-tab`；视觉状态 `autonomous` / `workmodemenu` |

⚠️ **不要把检索搬进扩展**：薄层只允许「读文件 + 放一条消息」（[实施-01 §1](../plan/实施-01-默认pi架构迁移.md) 的允许钩子白名单与用途约束）。
