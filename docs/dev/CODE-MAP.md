...
| `shared/handoff-notice.ts` | 128 | **交接状态一行提示的判定（实施-14 F5 / H5）**：`handoffNoticeOf`（进行中 > 失败（比最近成功晚）> 近期成功（60s 窗口）> 不显示）、`handoffReasonText`（原因翻人话，未登记的原样回传）。`result-mismatch` **不算**失败（只是磁盘上有别人的结果文件）| `HandoffNote.tsx`；单测 `test-handoff-notice.mjs` |
| `renderer/src/components/chat/HandoffNote.tsx` | 62 | **输入区上方的一行非阻塞交接状态（实施-14 F5）**：轮询 `getHandoff()`（4s），进行中 / 已整理 / 整理未完成 + 重试 / 停止；无可报之事返回 `null` 不占地方 | `App.tsx`（QuestionPanel 与 Composer 之间）；样式在 `styles/composer.css` 的 `.handoff-note` |
