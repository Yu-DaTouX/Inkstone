...
- **默认开启**：上下文工作集预算、Tool Sweep、`context_recall`、压缩接管框架及失败降级。
- **默认关闭**：`episode-fold` 的 TaskState 生成/注入、Deep Context Pass 1；只有策略显式打开才会调用对应模型路径。
- **主动移除**：记忆存储与 `remember/recall/forget`，以及旧会话树 `get_tree` 浏览链路；不要把它们画成缺失实现。
