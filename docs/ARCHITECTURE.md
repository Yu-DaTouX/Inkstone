...
- **主动移除**：旧全局记忆系统（存储、`remember/recall/forget`、记忆扩展、提示词注入）与旧会话树 `get_tree` 浏览链路；不要把它们画成缺失实现。**「项目知识」是独立新功能**（`docs/plan/实施-03-项目知识与旧记忆清理.md`，**S0–S5 已实施：存储 + 检索 + 注入 + `yan knowledge` CLI + 设置页**），既不等于恢复旧记忆，也不自动导入旧存储。
