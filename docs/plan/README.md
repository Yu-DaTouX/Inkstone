...
| [03](实施-03-项目知识与旧记忆清理.md) | 旧记忆残留清理 + 新项目知识 | [实施方案-B](../archive/2026-09-18-实施方案-B-旧记忆清理与项目记忆.md) | S0–S6 | 01-S2 | **S0–S2 已完成**（2026-09-19：S0 边界措辞复核（无需改动）+ S1 旧记忆残留清理 —— 4 处代码注释/文案 + 3 处文档改成当前事实，并建立**保护集**防关键词误删；S2 **存储层** `src/shared/project-memory.ts` + `src/main/project-memory-store.ts`（不可变 revision + 原子 manifest + CAS + 排他锁 + 墓碑，单测 **+153**，证据 [证据-03-S2](证据-03-S2-项目知识存储.md)））。**S3–S6（检索 / `yan knowledge` CLI / 设置页 / 跨会话与包）未开工** |
...
- **旧记忆系统不恢复**：记忆存储、`remember`/`recall`/`forget`、记忆扩展与提示词注入都不恢复；
  用户遗留数据不删。03 的「项目知识」是**独立新功能**（**尚未实施**），既不等于恢复旧记忆，也不自动导入旧存储。
