...
> **2026-09-21 状态增量**：S6b-2 新增 staging manifest 精确文件集复核，并补上 `mcp-package` 的精确 bin 解析、官方 SDK `tools/list` smoke、项目范围 stdio 登记 / 复核 / 幂等重放、受保护环境变量拒绝和超时清理；本轮又补上独立 Skill 目录条目适配（必须有版本 / commit、逐文件 URL 与 SHA-256）和受限读取器（来源白名单、UTF-8 / 大小 / 跳转 / hash 复核），离线 `test:skill-source` 已通过，但尚无真实外部目录检索证据。S7 已补 MCP 私有服务 runner 项目隔离（全局项保留、本项目私有项只对匹配项目可见；无项目身份 fail-closed）、能力设置页、三档模式策略、显式核验与取消 / 重连；`typecheck` / `build` / 单测 **4158/4158**，`capsettings` / `mcpcli` / `capcli` 真实运行通过；`visual:matrix` 已生成并实际看图核对能力页深浅主题，4 张 S7 截图均溢出 `0px`。`dist:dir` + 最新 `test:packaged` 已在解包、便携版和全新 NSIS 安装后 EXE 中取得完整运行探针；安装器侧栏使用 `build/installerSidebar.bmp` 并随新产物接入。外部候选整链、依赖、纯自定义工具包的限制仍待做。08-S0 的隔离 Electron `remoteroutes` 已全绿并纳入 `check`，本轮未复跑。**06-S2 的 A/B 基准已完成，不再以“把 A 组丢失率抬到 20–40%”为待办**：v7 的 A 组为 11.1%，丢失集中于单一任务，继续校准目标已被证据证伪。最终全量 `check` 优先使用已注册且确认能调用工具的本地 llama.cpp 模型；本轮已现场核对 `GET /v1/models` 与 `POST /v1/chat/completions`，并完成真实工具调用，当前 `local/qwen3-local` 可用。详见 [HANDOFF 本轮记录](../dev/HANDOFF.md)、[S6 技术预检 §6](证据-04-S6-技术预检.md) 与 [06-S2 证据 §6](证据-06-S2-N21-9基准-2026-09-19.md)。
>
