...
| 六栏 | 当前结论 |
|---|---|
| 实现 | H-1、H-2、C-1 已交付（计时口径纯函数 + 右栏资源保留 + 大窗口试行档）；DSH 的文件链接解析、`TabDomain` 生命周期和压缩事务顺序尚待搬运 / 接线；H-3、H-4、H-6、H-7、H-8、H-9、H-10、H-11、C-2、C-3、C-4、C-5、C-6 尚未闭环。 |
| 自动检查 | 本片 `npm run typecheck` / `build` 通过，`test:unit` **4227/4227**（含 `test-turn-timing.mjs` 与 C-1 的 6 条试行档断言）；其余切片的单测与文档审计待各片补齐。 |
| 真实运行 | H-1 已有 cost 0 与 cost 1 两条隔离场景（后者用本地模型 `local/qwen3-local`）；H-2 有 `rightresources`（cost 0，21 条）；C-1 的写入 / 禁用 / 切模型不继承已由 `contextbudget`（cost 0）真实窗口取证。 |
| 视觉验收 | H-1 `matrix-turnfooter-*`、H-2 `matrix-rightresources-*`（dark 引 h2b）、C-1 `matrix-ctxmodelpresets-*` 深浅各一张并看图核对；C-2 / C-5 的上下文窗口证据仍未取。 |
