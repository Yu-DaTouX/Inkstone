...
| 六栏 | 当前结论 |
|---|---|
| 实现 | H-1 已交付（计时口径抽成纯函数 + 用时条悬停说明）；H-2、C-1 已落下代码但缺证据；DSH 的文件链接解析、`TabDomain` 生命周期和压缩事务顺序尚待搬运 / 接线；H-3、H-4、H-6、H-7、H-8、H-9、H-10、H-11、C-2、C-3、C-4、C-5、C-6 尚未闭环。 |
| 自动检查 | 本片 `npm run typecheck` / `build` 通过，`test:unit` **4227/4227**（新增 `test-turn-timing.mjs`）；其余切片的单测与文档审计待各片补齐。 |
| 真实运行 | H-1 已有 cost 0 与 cost 1 两条隔离场景（后者用本地模型 `local/qwen3-local`）；H-2 的资源保留、C-1 的设置接线尚未取证。 |
| 视觉验收 | H-1 已有 `matrix-turnfooter-*` 深浅两张并看图核对；H-2 与上下文窗口证据仍未取。 |
