...
| 1 | H-1 | 整轮时间与回合页脚 | 方案 §4.1–4.2、§9 S1 | — | 已实现，待行为 / 视觉证据 |
| 2 | H-2 | 右栏资源保留 | 方案 §5.3、§9 S2 | — | 已实现，待行为 / 视觉证据 |
| 3 | C-1 | 大窗口模型级试行档 | 审阅 §5.1 | — | 已实现，待公式 / 设置页 / 端点证据 |
| 4 | H-6 | 回合计时契约与 usage 口径 | 方案 §4.2 | H-1 | 未开始 |
| 5 | H-7 | 时间呈现统一与可访问 | 方案 §13.1 | — | 部分已有，待补齐 |
...
| 切片 | 已落地内容 | 当前证据 | 状态 |
|---|---|---|---|
| H-1 整轮时间与回合页脚 | `agent.ts` 分开记录 agent 回合起点与首 token 起点；`elapsedMs` 覆盖工具 / 重试在内的整轮，`speed` 仍按首 token 计算；`turns.ts` 保留助手回合最后时间戳；`TurnView` 移除助手正文顶部的品牌 / 档位 / 步数标签，把时间、步数和耗时放到回合底部 | `npm run typecheck` 已通过 | 已实现，待真实窗口 / 视觉证据 |
| H-2 右栏资源切换 | 切换工具 / 审查 / 浏览器 / 文件时不再自动关闭其他可返回资源；浏览器原生视图只随活动窗口显隐；显式关闭仍会关闭对应资源 | `RightPanel.tsx`、`store.ts` 已改；尚未跑本片专门 live 场景 | 已实现，待行为与视觉证据 |
| C-1 大窗口模型级试行档 | 新增当前精确模型的 `balanced = 600K / 0.7` 与 `long = 700K / 0.7`；设置页只写入 `provider/model` 覆盖；运行时未登记至少 800K 窗口时不提供“可直接使用”的按钮 | `context-policy.ts`、`ContextTab.tsx` 与中英文文案已改；尚未跑单测 / 真实模型 | 已实现，待公式、设置页和端点证据 |
...
**H-1 · 整轮时间与回合页脚（位次 1，已实现待证据）**

- 内容：去掉助手正文顶部“砚 / N 步 / 标准”整行；底部只保留复制、用时、完成时刻、更多；无点赞 / 点踩 / 评分入口。
- 出口：多次工具、重试、停止、重载后整轮时间仍正确；无工具无推理的普通回答直接从正文开始；真实窗口 + 深浅主题截图。
- 禁区：不把工具等待计入生成速度；本期不联动修改工作模式 / 回复详细度 / 工作区模式的实际值。
- 缺口：缺本片 live 场景与视觉证据（旧右栏 / 会话截图不能替代）。
...
**H-7 · 时间呈现统一与可访问（位次 5，部分已有）**

- 来源：方案 §13.1。
- 现状：`TurnView.tsx` 已有 `<time datetime>` + `Intl` 完整时间 `title`（用户消息与助手底部共用 `TurnTime`）。
- 缺口：键盘聚焦 / 触摸也能读到完整时间（不能只有鼠标悬停）；消息区与子代理列表共用同一 formatter；
  发送队列中的消息标“待发送”；若要在界面上展示“点击发送的时刻”，另立 `submittedAt`，
  不与 pi 消息落盘时间混用。
- 落点：`TurnView.tsx`、`src/shared/ipc.ts`、`normalize.ts`、子代理列表格式化处。
- 禁区：缺失时间不显示伪造值；复制 / 分叉 / 历史恢复保留原消息时间语义，不用组件挂载时刻。

#### B 组 · 上下文与 1M（C 系列）

**C-1 · 大窗口模型级试行档（位次 3，已实现待证据）**

见 §3.2。待补：纯逻辑单测（不同窗口 / 输出预留 / 安全余量下的最终预算）、设置页接线证据、
真实端点证据；`600K / 700K` 仍是可回退试行档，不是全局默认或性能承诺。
...
**H-2 · 右栏资源保留（位次 2，已实现待证据）**

见 §3.2。待补：本片专门 live 场景（切工具 → 浏览器 → 文件 → 回浏览器，原页面仍在）与视觉证据。
...
| 六栏 | 当前结论 |
|---|---|
| 实现 | H-1、H-2、C-1 已落下首批代码；DSH 的文件链接解析、`TabDomain` 生命周期和压缩事务顺序尚待搬运 / 接线；H-3、H-4、H-6、H-7、H-8、H-9、H-10、H-11、C-2、C-3、C-4、C-5、C-6 尚未闭环。 |
| 自动检查 | 本轮 `npm run typecheck` 已通过；单测、构建和文档审计需在首批代码稳定后继续跑。 |
| 真实运行 | H-1 的整轮工具耗时、H-2 的资源保留、C-1 的设置接线尚未在本轮新的隔离 Electron 场景中取证。 |
| 视觉验收 | 尚未为本片新增截图或人工看图；已有旧右栏 / 会话截图不能自动替代本片的页脚、资源切换与上下文窗口证据。 |
| 应用与包 | 尚未按本片重新运行 `启动-砚.cmd`、`dist:dir` 或打包探针；不把旧 `out/` / `release/` 文件当成本片证据。 |
| 剩余限制 | 未完成真正的 `WorkbenchState`、多标签 / 多文档、终端入口、压缩状态可观测性和真实 1M 端点验证；600K / 700K 仍是模型级试行参数，不是性能承诺；`usage` 语义与 sweep 阶段线口径尚未核实。 |
