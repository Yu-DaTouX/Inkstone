...
2026-09-23 规划补充：右栏默认开始页、工具页、原生视图遮挡、子代理交互按本文 §5 继续细化；新增菜单/目标/磁贴由实施-12 持有，全局 UI 精修由实施-13 持有。本轮只完善方案，不把新切片标为已实施。
...
[并行代理编排](../../archive/plan/编排-并行代理分工-2026-09-19.md)（本文 §3.7 只做索引，不复制它的规则）。
...
| 4b | H-6b | 稳定逻辑回合、等待分段与 usage 聚合 | 方案 §4.2 出口 1/3/6 | H-6 | **已交付**（2026-09-23：崩溃→中断、稳定 logicalTurnId + runId 覆盖/累加、waitSpans 区间并集、usage 按消息聚合；六栏见 HANDOFF） |
...
| 7 | C-2 | 压缩可观测性 | 审阅 §5.2(3)、§6 | C-4 | **已交付**：回收比例 / 此后新增量 / 「待测」口径（C-2a）+ 三类动作分开统计（C-2b，扩展写账本 / 宿主读 / 界面分行，2026-09-24）；剩余：阈值标定归 C-6 |
| 8 | C-6 | 三类整理门槛与防抖标定 | 审阅 §5.2、§3.3、§5.1 | C-4、C-2 | **已交付**（2026-09-24，六栏见 HANDOFF）：“到线不再等于无条件整理” + 大窗口尾部 64K–96K + 折叠准备线 `max(48K, 软线×60%)` + 压缩后防抖（回落 80% / 新增 `max(16K, 软线×5%)` / 5 分钟重试）；真实 1M 端点归 C-3 |
| 9 | C-5 | 上下文窗口 UI 分层 | 审阅 §6 | C-2 | **已交付**（分母口径：主值走物理尺度 + 工作集单独一行 + 刻度改窗口尺度；档位名与容量来源已于 2026-09-22 补齐） |
| 10 | C-3 | 大窗口策略验证矩阵 | 审阅 §7 | C-1、C-6 | 未开始 |
| 11 | H-3 | 工作窗口状态模型 | 方案 §5.3、§9 S2 | H-2 | **H-3a 状态契约、H-3b 开始页均已于 2026-09-23 收口**（版本 2 契约、资源身份/kind 分离、固定开始/工具页）；多文档滚动 / 导航 revision 与完整资源身份仍归 H-4 / H-9 |
| 12 | H-4 | 文件与文档窗口 | 方案 §3、§8、§9 S3 | H-3 | **已交付**：解析 / 呈现、Markdown 阅读 / 源码切换、范围高亮、大文件窗口化、变化提示、资源身份、文件树联动定位、文档相对路径、定位父目录（见 HANDOFF「H-4a」/「H-4b」及 2026-09-24 两条 H-4 条目）；残余：历史消息保留来源根目录、窗口化只覆盖已读 2MB、真实截图矩阵 |
| 13 | H-9 | 浏览器窗口融合 | 方案 §6、§9 S4 | H-3 | **H-9a 原生视图与 overlay 协调器（2026-09-23）、H-9b 导航状态 / 错误体验 / 多标签归属（2026-09-24）已交付**；残余：网页 `window.open` 后台开标签未端到端取证、下载列表 / 权限按 origin 分组未做 |
| 14 | H-10 | 子代理展示与累计统计 | 方案 §7、§9 S5 | H-3 | **H-10a / H-10b 已收口（2026-09-23）**：归属选择器、三页签、执行态与审阅态分离、累计 usage；跨会话推送与长任务上滚仍只由单测 / 事件回放覆盖 |
| 15 | H-8 | 图标接入统一 | 方案 §13.2 | H-3（形态确定后） | **a/b/c 已收口（2026-09-23）**：程序图标全部来源化 / 随主题变色 / 可访问名 81/81；浅色适配 + 状态样式单一来源。**剩外壳图标包验收**（静态核验已进 `check`，装完才看得见的 8 项未验）。 |
| 16 | H-11 | 交互终端 | 方案 §8、§9 S6 | H-3 | **已交付**（2026-09-24，六栏见 HANDOFF）：宿主 PTY 服务 + xterm 表面 + 工作窗口资源，原生依赖包内真跑（`test:packaged`） |
...
- **已交付（H-6b，2026-09-23）**：
  1. **稳定 `logicalTurnId`** = 用户消息 id（`anchorId`），并新增 `runId` 区分「同一段工作的两次快照」与「自动继续的另一段」：
     同一 run 后者胜、不同 run 用时**相加**（旧记录无 `runId` 时保持后者胜，不把历史翻倍）；
  2. **`waitSpans`**：每段工具调用的真实起止区间（`toolWaitSpans`），总长走**区间并集**（`waitSpansMs`，并行分支不重复计），落盘并在回合页脚的用时 tooltip 里显示「其中等工具 N」；
  3. **usage 按消息去重后聚合**（`turnUsageOf`）：每条助手消息的 usage 是该次请求的独立用量 → 相加；回合内有请求没报用量时合计标为下限（`≥` + tooltip 说明）；
  4. 六栏证据（实现 / 单测 / cost 0 与 cost 1 真实运行 / 视觉）见 [HANDOFF](../../dev/HANDOFF.md) 的「实施-11 H-6b」节。
- **仍未交付（H-6b）**：
  1. **跨会话链交接**（handoff 换会话文件）的 `logicalTurnId` 归属未取证 —— 自动继续已由 `goalloop` 端到端验过，跨文件需要新场景；
  2. 中途快照的**产生**仍是重启前追加（可复现），非真 SIGKILL —— 强杀时最后一条快照是否已落盘未实测；
  3. `waitSpans` 目前只服务「等工具」展示，方案里的「等待用户（授权 / 补充回答）」未单列。
  4. 父回答结束后台子代理继续时父用时冻结（agent_settled 已冻结），但**子代理独立计时**属 H-10，不在本片。
...
- **已交付（C-2b，2026-09-24，证据见 HANDOFF）**：三类动作分别统计 —— 扩展写 `<YAN_DATA_DIR>/context-actions/<sessionId>.jsonl`
  （清扫 / 状态刷新的 applied / skipped / rejected / injected / failed，含条数与估算 token），
  宿主新增 `yan:contextActions` 读并聚合，界面在上下文详情里**分三行**显示（清扫 / 状态刷新 / 整轮压缩），
  未发生写「未发生」。账本**不重抄**一份 compaction（那一类仍以 pi 事件为准）。
  真实取证：`contextsweep`（cost 1）读到清扫 6 次 / applied 4 次 / 整理 5 条。
...
**C-6 · 三类整理门槛与防抖标定（位次 8，已交付）**

- 来源：审阅 §5.2、§3.3、§5.1。
- **已交付（2026-09-24，六栏见 HANDOFF）**：
  1. **前置核实的结论**：pi 0.85.1 的自动压缩是 `contextTokens > contextWindow − reserveTokens`（`reserveTokens` 默认 16384），
     在 agent harness 里每轮助手应答前触发，另有溢出路径；请求的 `maxTokens` 取能力字段、**不是**每次预留 393216。
     所以 1M 下可用输入预算 ≈ 963k，600K / 700K **不需要**下调（单测钉住 `contextBudget()` 的输出）。
  2. **Tool Sweep**：到工作集时不再把收益门槛清零 —— 改用 `DEFAULT_SWEEP.forcedMinReclaim*`（默认等于普通门槛），
     仅在策略显式给 0 时回到旧行为；大窗口档（工作集 ≥ 512K）近期尾部放宽到 64K–96K（`recentTailFor`）。
  3. **状态摘要**：准备线 = `max(48K, 整轮软线 × 60%)`（`FOLD_PREPARE_RATIO`，600K 档约 360K）；
     显式给 `minTokens` 时不抬高（保测试 / 标定通道能精确控制分支）。
  4. **整轮 compaction**：用 `compaction_end` 的 `estimatedTokensAfter` 做基准，在“已上膛”之外再加一条
     —— 回落软线 80% 以下、或此后新增 ≥ `max(16K, 软线×5%)`（夹在软线内）、或到 5 分钟重试窗口；
     硬压力（`emergency`）不受防抖影响。低回收 + 低新增时界面写「主要为不可压缩基础开销」。
- **仍归 C-3**：真实约 1M 端点上的参数标定与多档对比（本片只把机制与候选值落地并逐条验证）。
- 原始候选记录（标定前，保留以备 C-3 对照）：
...
- 禁区：
  ...
  - **不得删除** 实施-06-S3 那个“成功后重新上膛”的修复 —— 新增回收量 / 增长量条件是**额外**条件，
    并须保留低线压力反向测试（`contextpressurelow`）——已按此实现并通过（见 HANDOFF）；
...
- **未交付**：
  1. ~~默认行里的**档位名**（600K / 700K 试行档）未写出来~~ —— **已交付（2026-09-22）**：
     右栏新增 `ctx-preset` 行（「档位 均衡 · 600K（写在当前模型上）」），判据是
     `shared/context-policy.ts` 的 `largePresetOf()`（设置页按钮与右栏共用同一函数）；
  2. ~~展开项里的**容量来源**仍只在设置面板~~ —— **已交付（2026-09-22）**：
     详情新增 `ctx-source-line`（「容量来源 模型级 · deepseek/…」），文案与设置页 `ctx-source` 同源；
  3. 三类动作分别统计（sweep / fold / compaction）仍归 **C-2b**。
- **⌛ 一次误报，已自查更正（2026-09-22 晚）**：本片曾登记「详情展开后一次 store 推送会让
  `ctx-details` 消失、右栏本地展开态可能丢失」。拆到底后确认**那是探针自己的 bug**，不是产品缺陷：
  该探针更早的步骤已经展开过详情，而新加的那段又**无条件 `click()`** 一次 —— 收起是被探针自己点出来的；
  紧接着的读取还跑在 React 提交之前，读到旧 DOM，反而让断言「通过」。
  现在的判据是幂等的（已展开就不动它）+ 等稳定后再读，并留下了真护栏：
  **进入本节时仍展开 / 静置 400ms 仍展开 / store 推送后仍展开 / 再点一次才收起**。
  根据：`MutationObserver` 只报了 `child(-1/+0)`（详情块自己）、`ctx-main` 未被替换；
  临时插桩的 `onClick` 调用栈总是指向那个按钮（验完已删）；未发现卸载 / 重挂载。
- 教训已归 `docs/dev/MAINTENANCE.md`：「探针里『点一下再立刻读』会读到未提交的 DOM」。
...
**H-4 · 文件与文档窗口（位次 12，已交付；残余见 HANDOFF）**
...
- **已交付（H-4c / H-4d，2026-09-24，证据见 HANDOFF）**：资源身份（`projectId + workspaceRoot + canonicalPath`）与多文件标签、文件变化提示（只 stat、不自动重载）、文件树联动定位（自动展开父目录并标为当前）、范围高亮（`#L42-L60` 按真实行号）、大文件窗口化（>2000 行时只切出目标行前后窗口）、文档内相对链接按**文档目录**解析、缺失态保留原路径 + 重试 + 定位父目录。
- **仍缺（单独切片）**：历史消息保留来源根目录 / 工作树（现在聊天引用统一按当前会话 cwd 解析）；窗口化只覆盖已读的 2MB；真实窗口截图矩阵。
...
- 第二阶段：导航状态与错误体验 —— 以真实事件（`did-navigate` / `did-navigate-in-page`）为已提交 URL 真源，
  ...
  **2026-09-24 已交付（H-9b，证据见 HANDOFF）**：另有纯模块 `shared/browser-navigation.ts`（失败判定 / 草稿取舍 / 出错地址保留）、
  `did-fail-load` 主框架归属、`openBackgroundTab`（网页弹窗不夺焦点）、多标签错误归属。
  未端到端取证：网页 `window.open` 的真实触发（沙箱里没有可控本地页面）；下载列表与按 origin 分组未做。
...
2026-09-23 细化：已有右侧 detached 列表与回合归属入口是基础；新形态为标题入口 → 紧凑列表 → 同一 runId 的右栏详情，详细设计见本文 §5 和侧栏设计 §6。不要把“未开始”理解为子代理执行链或已有 UI 完全不存在。
...
- 来源：方案 §8、§9 S6。
- **已交付（2026-09-24，六栏见 HANDOFF）**：宿主 `src/main/terminal.ts` 用 **node-pty**（N-API 预编译，无需 Electron 重编）
  管会话：开始 / 写入 / 缩放 / 关闭 / 退出 + 有界回滚缓冲 + 输出序号；`yan:terminal:*` 七个 IPC 与 `preload.terminal`；
  渲染端 `TerminalSurface`（xterm + FitAddon，主题从 `tokens.css` 读，attach 前把实时块排队以避免快照/流交错）；
  接入工作窗口作为第五类资源（开始页入口 / 标签栏 / `+` 菜单）。包内真跑见 `test:packaged`（`asarUnpack` + `npmRebuild: false`）。
...
历史波次见[归档编排](../../archive/plan/编排-并行代理分工-2026-09-19.md)；当前共享文件按本节单 owner 规则和[本轮调度](执行-2026-09-23-审核融合与任务提示词.md)执行。下面列**本队列会撞到的地方**：
...
| 六栏 | 当前结论 |
|---|---|
| 实现 | H-1、H-2、H-3a、H-3b、H-6（+H-6b）、H-7、H-9a、H-10a、H-10b、H-8a·b·c、C-1、C-4、C-5、C-2b、C-6、H-11、H-4、H-9 第二阶段已交付（2026-09-24）；C-3 仍需明确允许与额度，H-8 只差 Windows 外壳 / 安装器包验收。 |
...
| 剩余限制 | 当前 `WorkbenchState` 已是版本 2 契约（资源身份 / kind 分离、固定开始页），但完整多文档工作台仍未闭环；终端入口、真实 1M 端点、外部候选能力整链与 Android 客户端仍未交付；600K / 700K 仍是模型级试行参数，不是性能承诺；H-6b 保留真强杀快照边界与跨会话链回合归属，C-2b / C-3 / C-6 仍待做。 |
...
## 5. 2026-09-23 右栏与子代理细化切片

本节是 H-3/H-9/H-10 的细化执行卡，替代把它们当作一次大改的做法；不重开已完成的 H-2，不覆盖其余 C/H 队列。详细交互唯一来源：[侧栏设计](../../design/active/方案-侧栏工作台与子代理交互-2026-09-23.md)。新增左栏/目标/磁贴与全局视觉分别见 [实施-12](实施-12-侧栏交互与可移动工具磁贴.md)、[实施-13](实施-13-全局UI精修与设计验收.md)。

### H-3a · 完善工作台状态与接口（✅ 状态契约已收口 2026-09-23）

输入：workbench.ts、RightPanel 的 windowView/workbench 两层状态、store 的资源 owner。增量：增加开始页语义，资源 ID 与页面 kind 分离；一个活动标签真源；订阅保存按所属会话 key 固定，不把 A 布局写到 B；冻结对接 ToolLayout 与 overlay blocker 的接口。

资源身份：文件继续按 H-4 的 project/workspace/canonicalPath；子代理 `subagent:<runId>`；浏览器用实际 page/tab 身份；审查取实际审查对象身份，不用显示标题做 key。kind 只判断渲染器，不决定唯一资源。历史 v1 按需迁移：保留用户宽度/展开和可恢复资源，未知项回开始页；pending 会话转稳定身份只能迁移一次，不能共享临时会话布局。异步打开用 requestId + sessionKey 校验，失败页给重试，不无限 pending。

出口：A/B 切换、同名不同根文件、两个子代理、关闭活动/非活动标签、未知旧布局、延迟返回、pending→真实会话的纯函数与 cost 0 Electron 证据。资源未接完可记“状态契约完成”，不得称多文档完整交付。文件域：workbench、store、RightPanel，shared/ipc 必要接口只由 owner 写。

> **2026-09-23 收口**：`workbench.ts` 升到 version 2（资源身份/kind 分离、`start` 固定页、显式版本迁移、唯一活动页真源、pending 一次性采用、异步打开 `requestId+sessionKey` 守卫）；`RightPanel` 删掉第二份 `windowView`，布局按所属会话 key 落盘（修切会话写错会话的竞态）。单测 4785/4785（本片 +22），`test:live -- rightresources` 全绿。`start` 页渲染器与默认页切换归 H-3b；多文档/滚动/导航 revision 仍归 H-4/H-9。证据见 [HANDOFF](../../dev/HANDOFF.md) 顶部。

### H-9a · 原生视图与 overlay 协调（✅ 协调器已收口 2026-09-23）

前置 H-3a。列出并收敛全部 browser.setVisible 调用；菜单/目标/拖动/设置/子代理使用 blocker 集合，只有“当前会话、活动网页、右栏展开、无遮挡”才可显示。清理一个浮层不直接 setVisible(true)。

出口：网页→目标→另一菜单→依次关闭；收右栏/换会话/关资源/慢打开返回；125/150% bounds；隐藏时网页资源保留，打开时不重建或丢导航。重点移除 SubagentPreview 清理时无条件恢复网页的路径。cost 0 可验证显隐接线，必须补真实窗口看图确认遮挡。文件域：BrowserSurface、RightPanel、SubagentPreview、App 的现有 overlay 协调处；宿主 browser 权限/网络边界保持。

> **2026-09-23 收口（协调器部分）**：唯一判定 `shouldShowBrowser`（四条件）+ `OverlayBlockers`（token 计数）落地；设置 / 工具菜单 / 子代理详情 / 收起右栏已接入，`SubagentPreview` 无条件恢复路径已删。`test:live -- overlayblockers`（cost 0）全绿，`rightresources` 回归通过。**未完成**：目标浮层（U-3a）与项目菜单（U-2）还不存在，四层依次关闭的完整链待它们落地；OS 级真窗口遮挡截图待补（capturePage 不含原生视图）。证据见 [HANDOFF](../../dev/HANDOFF.md) 顶部。

### H-3b · 默认开始页与工具页（✅ 已收口 2026-09-23）

前置 H-3a/H-9a。新增开始页，不新造第二个工具栏；入口为审查/浏览器/文件/工具，终端由 H-11 可用性决定。固定开始/工具导航，资源标签按 ID 激活；新会话从开始页起，旧会话恢复有效活动页；最后资源关闭回开始。

收起整个右栏只隐藏资源，包括原生网页；展开恢复当前页；扩大与恢复宽度分开。不得沿用旧“收起只隐藏工具栏、网页仍满列”语义。宽度用户偏好与页面临时宽度区分，退出审查/扩大回原值。

出口：首次进入、旧布局恢复、从开始页逐项打开、最后标签关闭、扩/收、窄矮窗入口可达；所有可点击项有实际动作，快捷键来自真实绑定。rightresources 等回归+真实深浅图。后续真正多文档、滚动/导航 revision 等 H-3 剩余出口继续存在，本片不自动完成它们。

> **2026-09-23 收口**：右栏新增固定「开始」页（审查/浏览器/文件/工具四个入口，终端未接入不放灰按钮）；新会话默认停在开始页，关闭最后一个资源回开始页；收起整个工作栏连原生网页一起隐藏（不再沿用“只藏标签、网页满列”）；工具页与开始页均为固定导航。单测 4800/4800，深/浅真窗口截图已看图；已修复 tools/todos/layout/browser/fs/fsedge/settings/vheight/context/quota/plusmenu/goallinks/logs/topbar/panels 等旧默认页假设。窄窗截图未出（capturePage 不稳）已单列。证据见 [HANDOFF](../../dev/HANDOFF.md) 顶部。

### H-10a · 子代理信息架构（✅ 已收口 2026-09-23）

前置 H-3a/b、H-9a、U-3a 短标题规则。先实现按父会话/明确链归属的 run selector，再接标题计数与列表；attached/detached 按 runId 去重，保留 parentMessageId 回合短入口。新任务不抢当前资源焦点；显式启动成功可以打开其详情。

详情由工作台资源标签承载，概览/过程/变更复用现有能力。清晰区分执行成功、审阅待处理、合并冲突。关闭/隐藏不停止、不 discard；停止操作有 pending/错误恢复；启动失败保留输入且不静默降低隔离。首期不做持续对话、嵌套、并发调整。

出口：0/1/20 个任务，各执行状态与审阅状态；长任务、过程上滚、新输出、跨会话推送；一个 run 多入口打开只有一份详情；只读无合并；关闭详情任务继续；启动重复点击只有一次请求。cost 0 UI fixture 与既有 subagent 接线；真实工作树/停止/合并行为按 CASES 成本与授权单列验证，不能用 fixture 冒充。文件域：SessionHeader、SubagentList、SubagentPreview/Details、TurnView、相关 selector；RightPanel owner 接线。

> **2026-09-23 部分收口**：`state/subagent-view.ts` 固定会话归属/去重/分组（attached/detached/foreign/unattributed）；列表按当前会话显示 + 四档筛选；详情升级为工作台资源标签 `subagent:<runId>`（多入口同一份）。**未完成**：概览/过程/变更三页签、执行态与审阅态独立展示、0/1/20 与跨会话推送专项探针。单测 4811/4811，subagent 探针结构断言全绿（模型输出失败属环境）。证据见 [HANDOFF](../../dev/HANDOFF.md) 顶部。
>
> **2026-09-23 收尾**：三页签（概览/过程/变更）与执行态/审阅态分离已落地；列表启动防重复点击；0/1/20 与重复推送选择器用例补齐；H-10b 累计 usage 同批交付。单测 4831/4831。跨会话推送与长任务上滚仍只由单测/事件回放覆盖。证据见 [HANDOFF](../../dev/HANDOFF.md) 顶部。

### H-10b · 累计 usage（✅ 数据层已收口 2026-09-23）

前置：冻结 SubagentRun 可选统计字段。定义 input/output/cache 含义和 unknown；每条消息以稳定 ID 接收累积快照/最终值，重复事件不加倍，流式增量不与 final 重复求和；持久化统计独立于有界 transcript。旧记录不能反推精确用量时显示未知。

出口：超过转录上限、重复 final、重放、缺 usage、取消/失败、归档再读、旧记录均有单测；cost 0 宿主事件回放验证持久化，真实 provider 计数另取证。父回合总 usage 仍属 H-6b，不能简单把并行耗时相加。文件域：src/main/subagents.ts、SubagentRun 契约/存储、相关测试。

> **2026-09-23 数据层收口**：`SubagentRun.usage?: SubagentUsageTotals`；`shared/subagent-usage.ts` 按消息 id 保留最后一份快照并求和（流式→final 不翻倍、重放不翻倍、不同消息相加），`reportedMessages=0→null 未知`，旧记录兼容。单测 8 条 + `test-subagents.mjs` D17 假 RPC 事件回放。未做独立落盘→重启读回探针；真实 provider 计数需额度。

### 细化片的证据与分工

H-3a → H-9a → H-3b → H-10a 为右栏主线；H-10b 数据层可按文件域错开。共享 RightPanel/store/ipc/SessionHeader/i18n/测试注册只有一位 owner；不要按“文件/浏览器/子代理”同时派三个人修改同一 RightPanel。

每片依次记录实现、自动检查、真实运行、视觉、应用与包、剩余限制。2026-09-23 本节仅规划：未改代码、未执行测试或启动应用。新的完整 UI 验收见 U-6/V-6，包验收仍按实施-09，暂无发布计划不自动打包。

## 6. H-8 增补：浅色与动效（2026-09-23）

H-8 拆为 H-8a 盘点与主题修复、H-8b 交互状态动效、H-8c 后续工作台触点。a/b 可以先做，不依赖未来完整 H-3；c 仍跟随资源表面稳定后验收。H-8 整体不能因当前触点通过而全勾选。

- H-8a：清点实际用到的图标、颜色来源及 default/hover/selected/disabled/error/focus；SVG 优先 currentColor + 语义主题 token，品牌和多色产物单列。Orb 已有 auto 主题，先实测再修。深浅切换不中断状态；功能图形对比度目标 3:1。
- H-8b：统一 hover/focus/press/展开反馈，复用现有 motion token，建议 120–180ms；运行态才持续动，完成/取消后停止。减少动态效果下关闭位移与持续旋转，保留状态辨识；不靠图形动画单独表示成功或失败。
- H-8c：新增工作台/资源触点消费同一接口；Windows 外壳图标仍按包验收单列，不自动启动打包。

> **2026-09-23 收口证据（a/b）**：图标一律走 `Icon`（sprite 全部 `stroke="currentColor"`），状态色由所属控件给；跨模块只新增 `.ico` 的 `color` 过渡（`icon-state.css`，已接入 App.tsx，reduced-motion 压到 1ms）。chat.css 里 artifact / 图片进度 / 子代理内联卡引用的 `--text/--panel/--line/--muted` **在 tokens 里根本不存在**，共 31 处回退固定浅字/深底 —— 已映射为 `--fg / --bg-2 / --border(|--border-soft) / --fg-mute|--fg-dim`，状态色保持 `--accent/--ok/--err`。`test:live -- iconstate`（cost 0）实测：深浅两态正文/次要文字/图形对比度均达 4.5:1/3:1，`.ico` 过渡 0.12s，进行态动画在类名移走后停止。浅色真实截图见 `docs/design/preview/icon-l3-*`。
>
> **2026-09-23 H-8c 收口（现有工作台 / 资源触点）**：新探针 `test:live -- iconsurface`（cost 0，已进
> `npm run check`）实测：sprite 引用覆盖标题栏 / 会话轨 / 会话流 / 输入区 / 右栏工具 / 浏览器 / 设置各区域；
> **81 个图标钮 100% 有可访问名**（`aria-label`/`title`/文本）；sprite 的 **28 个 symbol 全是
> `currentColor`**（写死颜色属性 0 个）；按身份对位的 **37 个可见图标切主题后 37 个跟随变色**；
> 渲染尺寸只有 12/14/16 三档。内联 svg 只剩品牌/图形（标题栏 logo、会话流 48×48 徽标等）。
> **Windows 外壳 / 安装器图标**：2026-09-23 做了**静态核验**（脚本 `scripts/check-shell-icons.mjs`，
> 已进 `npm run check`）：`icon.ico` 含 16/32/48/64/128/256、`icon.png` 512×512 带透明通道、
> `installerSidebar.bmp` 164×314（NSIS 约束）、配置没把图标指向别处 —— 全部通过。
> 装完才看得见的 8 项（安装器 / 开始菜单 / 桌面 / 任务栏 / 卸载器 / 安装目录 / 高 DPI / 便携版）
> 列成 [包验收单](../../dev/包验收-Windows外壳图标-2026-09-23.md)，**未验**且本片不自动打包。
> 所以 **H-8 整体仍不勾选**：代码侧触点已收口，外壳图标这一列保持未验；后续新增工作台触点
> （H-3/H-4 稳定后）仍需按同一接口复核。

文件 owner：Icon/sprite、实际图标样式、必要调用点；不引入新图标库、不手改生成 sprite 后遗忘生成器。逐项记录尺寸、命中区、无障碍标签和实图结果。任务提示词见 [本轮调度 L3](执行-2026-09-23-审核融合与任务提示词.md)。
