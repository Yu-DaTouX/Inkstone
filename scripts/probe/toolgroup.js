...
;(async () => {
  ...
  /*
   * 高度口径：**25 行**（用户 2026-09-19：「改为 25 条」）。
   *
   * ⚠️ 2026-09-25 我一度把它改成「窗口高的四分之一」（25vh）去跟产物图片
   *    统一口径，用户报「工具栏变回之前的了」—— 已恢复成条数算法。
   *    工具组的最小单位是「条」；推理全文与单条详情是一段连续文本，
   *    它们用 min(70vh, 620px)，不跟这里同口径。
   */
  ...
  ok(
    rowsInView >= 25,
    '一屏能看到 25 条（不是被窗口高度卡住）',
    `实测 ${rowsInView} 条 / 行步进 ${rowStep.toFixed(1)}px`
  )
  ok(
