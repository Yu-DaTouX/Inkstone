 ...
 *   ② **有上限 + 退避**：默认连续 5 次，间隔 10s / 30s / 1m / 2m / 4m ——
 *      供应商抽风往往就是几分钟的事（上游 5xx / 过载 / 连接被掐），
 *      隔得太短只会在同一个坏窗口里烧额度，所以退避单调拉长到 4 分钟；
 ...
 * 连续自动继续的上限（用户发言即归零）。
 *
 * 5 次是 2026-09-23 用户拍板：供应商抽风时给到足够长的恢复窗口，
 * 又不至于整夜自己在那儿转。
 */
export const AUTO_CONTINUE_LIMIT = 5

/**
 * 每次尝试前的等待（毫秒），与 {@link AUTO_CONTINUE_LIMIT} 一一对应。
 *
 * 10s → 30s → 1m → 2m → 4m：单调递增。前面几次快（多数抖动几十秒内就好），
 * 后面拉开（真遇上长时间故障时，密集重试只会把额度烧在同一个坏窗口里）。
 * 最后一项会被复用（到上限前不会超过 limit 次）。
 */
export const AUTO_CONTINUE_DELAYS_MS = [10_000, 30_000, 60_000, 120_000, 240_000] as const
